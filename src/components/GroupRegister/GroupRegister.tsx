...
function GroupRegister({
    ...
    const isLast = data.groups && data.groups.length > 0 && group.key === last(data.groups)?.key;

    const handleDeleteGroup = () => {
        const newDataGroups = data.groups && data.groups.filter(
            (item) => item.key !== group.key
        );
        setData({...data, groups: newDataGroups});
    };
    const handleAddGroup = () => {
        setData({...data, groups: [ ...data.groups as IGroupRegister[], { name: "", key: uuidv4()}]});
    };
    const handleChangeGroups = (e: React.ChangeEvent<HTMLInputElement>) => {
        const target = e.target;
        const { value } = target;

        const newDataGroups =  data.groups && data.groups.map((item) => {
